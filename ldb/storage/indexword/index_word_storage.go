...
var zeroUint32Bytes []byte = com.Uint32ToBytes(0)
var zeroUint16Bytes []byte = com.Uint16ToBytes(0) // 索引件数的key
...
func NewWordIndexStorage(storeName string) *WordIndexStorage { // 存储器，文档，自定义对象
	...
	// 缓存有则取用
	subPath := "inverted" + com.PathSeparator() + "k"
	cacheName := storeName + com.PathSeparator() + subPath
	...
	if err != nil {
		global.LbLogger.Info(fmt.Sprintf("打开WordIndexStorage失败：%s", dbPath))
		panic(err)
	}
	store.leveldb = db
	...
	global.LbLogger.Info(fmt.Sprintf("打开WordIndexStorage：%s", cacheName))
	return store
}
...
func (s *WordIndexStorage) loadIndexedCount() {
	...
	if err == nil {
		s.indexedCount = com.BytesToUint32(bt)
	}
}
...
func (s *WordIndexStorage) SaveIndexedCount(count uint32) error {
	...
	return s.leveldb.Put(zeroUint16Bytes, com.Uint32ToBytes(count), nil)
}

// 取关键词索引当前的文档数
func (s *WordIndexStorage) GetTotalCount(word string) uint32 {
	wordBytes := com.StringToBytes(word)
...
func (s *WordIndexStorage) setTotalCount(word string, cnt uint32) error {
	return s.leveldb.Put(com.JoinBytes(com.StringToBytes(word), zeroUint32Bytes), com.Uint32ToBytes(cnt), nil)
}
...
func (s *WordIndexStorage) Add(word string, docId uint32) error {
	...
	seq++
	err := s.leveldb.Put(com.JoinBytes(com.StringToBytes(word), com.Uint32ToBytes(seq)), com.Uint32ToBytes(docId), nil)
	if err != nil {
		global.LbLogger.Error(fmt.Sprintf("保存关键词反向索引失败: %v", err))
		return err
	}
	...
	if err != nil {
		global.LbLogger.Error(fmt.Sprintf("保存日志反向索引失败: %v", err))
		return err
	}
	...
	if err != nil {
		global.LbLogger.Error(fmt.Sprintf("保存关键词反向索引件数失败: %v", err))
		return err // 忽略事务问题，可下回重建
	}
	// fmt.Println("创建日志索引：", docId, "，关键词：", word)
	return nil
}
...
func (s *WordIndexStorage) GetDocId(word string, seq uint32) uint32 {
	...
	s.lastTime = time.Now().Unix()
	b, err := s.leveldb.Get(com.JoinBytes(com.StringToBytes(word), com.Uint32ToBytes(seq)), nil)
	if err != nil {
		return 0
	}
	return com.BytesToUint32(b)
}
...
func (s *WordIndexStorage) Close() {
	...
	global.LbLogger.Info(fmt.Sprintf("关闭WordIndexStorage：%s%s%s", s.storeName, string(os.PathSeparator), s.subPath))
}
...
func onExit() {
	...
	global.LbLogger.Info("退出WordIndexStorage")
}
