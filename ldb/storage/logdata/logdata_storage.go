...
func NewLogDataStorage(storeName string) *LogDataStorage { // 存储器，文档，自定义对象
	...
	store.storeChan = make(chan *LogDataModel, global.LbConfig.Store.ChanLength) // 初始化管道，设定缓冲

	dbPath := global.LbConfig.Store.Root + com.PathSeparator() + cacheName
	global.LbLogger.Info(fmt.Sprintf("dbPath：%v", dbPath))
	db, err := leveldb.OpenFile(dbPath, nil) // 打开（在指定子目录中存放数据）
	if err != nil {
		global.LbLogger.Error(fmt.Sprintf("打开LogDataStorage失败：%v", dbPath))
		panic(err)
	}
	store.leveldb = db
	...
	global.LbLogger.Info(fmt.Sprintf("打开LogDataStorage：%s", cacheName))
	return store
}
...
func (s *LogDataStorage) readyGo() {
	for {
		...
		default:
			...
			for i := 0; i < global.LbConfig.Store.GoMaxProcessIdx-1; i++ {
...
func (s *LogDataStorage) saveLogData(model *LogDataModel) {
	...
	model.Id = com.Uint32ToString(s.currentCount) // 模型数据要转Json存，也得更新ID,ID用36进制字符串形式表示
...
func (s *LogDataStorage) createInvertedIndex() int {
	...
	if err != nil {
		s.muIdx.Unlock()
		global.LbLogger.Error(fmt.Sprintf("取日志模型数据失败：已索引件数: %d, 错误信息: %v", s.indexedCount, err))
		return 2
	}
	...
	var adds []string
	if docm.System != "" {
		adds = append(adds, "~"+com.ToLower(docm.System))
	}
	if docm.LogLevel != "" {
		adds = append(adds, "!"+com.ToLower(docm.LogLevel))
	}
	if docm.User != "" {
		adds = append(adds, "@"+com.ToLower(docm.User))
	}

	tgtStr := docm.System + " " + docm.ServerName + " " + docm.ServerIp + " " + docm.ClientIp + " " + docm.TraceId + " " + docm.LogLevel + " " + docm.User
	if docm.Detail != "" && global.LbConfig.Search.MultiLineSearch {
...
func (s *LogDataStorage) GetLogDataModel(id uint32) (*LogDataModel, error) {
	...
	s.lastTime = time.Now().Unix()
	bts, err := s.leveldb.Get(com.Uint32ToBytes(id), nil)
	if err != nil {
		return nil, err
	}
...
func (s *LogDataStorage) Close() {
	...
	global.LbLogger.Info(fmt.Sprintf("关闭LogDataStorage：%s", s.storeName+com.PathSeparator()+s.subPath))
}
...
func (s *LogDataStorage) loadMetaData() {
	...
	if err != nil || bytes == nil {
		...
		s.currentCount = com.BytesToUint32(bytes)
