...
func NewDocIndexStorage(storeName string) *DocIndexStorage { // 存储器，文档，自定义对象
	...
	// 缓存有则取用
	subPath := "inverted" + com.PathSeparator() + "d"
	...
	if err != nil {
		global.LbLogger.Error(fmt.Sprintf("打开DocIndexStorage失败：%s", dbPath))
		panic(err)
	}
	store.leveldb = db
	...
	global.LbLogger.Info(fmt.Sprintf("打开DocIndexStorage：%s", cacheName))
	return store
}
...
func (s *DocIndexStorage) AddWordDocSeq(word string, docId uint32, seq uint32) error {
	...
	err := s.leveldb.Put(com.JoinBytes(com.StringToBytes(word), com.Uint32ToBytes(docId)), com.Uint32ToBytes(seq), nil)
	if err != nil {
...
func (s *DocIndexStorage) GetWordDocSeq(word string, docId uint32) uint32 {
	...
	s.lastTime = time.Now().Unix()
	b, err := s.leveldb.Get(com.JoinBytes(com.StringToBytes(word), com.Uint32ToBytes(docId)), nil)
	if err != nil {
		return 0
	}
