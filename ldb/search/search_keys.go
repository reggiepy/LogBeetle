...
import (
	...
	"github.com/reggiepy/LogBeetle/ldb/storage"
...
func SearchWordIndex(storeName string, cond *SearchCondition) *SearchResult {
	...
	minDocumentId := com.StringToUint32("1", 1) // 时间范围条件内的最小文档ID
	if !com.IsBlank(cond.DatetimeFrom) {
		...
		if minDocumentId == 0 {
			...
			rs.Total = com.Uint32ToString(storeLogData.TotalCount())
			rs.Count = "0"
			return rs
		}
	}
	if !com.IsBlank(cond.DatetimeTo) {
...
func SearchLogData(storeName string, cond *SearchCondition) *SearchResult {
	allloglevels := com.Join(cond.Loglevels, ",")              // 合并多选的级别条件
	noLogLevels := com.IsBlank(allloglevels)                   // 无多选条件
	var rs = new(SearchResult)                                 // 检索结果
	storeLogData := storage.NewLogDataStorageHandle(storeName) // 数据
	totalCount := storeLogData.TotalCount()                    // 总件数
	rs.Total = com.Uint32ToString(totalCount)                  // 返回的日志总量件数，用10进制字符串形式以避免出现科学计数法
	rs.Count = com.Uint32ToString(totalCount)                  // 当前条件最多匹配件数
	rsCnt := 0                                                 // 已查到的件数
	...
	minDocumentId := com.StringToUint32("1", 1) // 时间范围条件内的最小文档ID
	hasMin := !com.IsBlank(cond.DatetimeFrom)
	hasMax := !com.IsBlank(cond.DatetimeTo)
	if hasMin {
	...
	if hasMax || hasMin {
		...
		rs.Count = com.Uint32ToString(maxDocumentId - minDocumentId + 1) // 估算的最大匹配件数
	}
	...
	if cond.CurrentId == 0 {
		...
		for i := max; i >= min; i-- {
			...
			if noLogLevels || com.ContainsIngoreCase(allloglevels, md.LogLevel) {
...
func findSame(cond *SearchCondition, minDocumentId uint32, maxDocumentId uint32, storeLogData *storage.LogDataStorageHandle, widxs ...*WidxStorage) *SearchResult {
	...
	rs.Total = com.Uint32ToString(storeLogData.TotalCount()) // 日志总量件数
	...
	rs.Count = com.Uint32ToString(minCount) // 当前条件最多匹配件数
