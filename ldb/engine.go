...
func (e *Engine) Search(cond *search.SearchCondition) *search.SearchResult {
	...
	for _, word := range kws {
		...
		if cnt < 1 {
			global.LbLogger.Info(fmt.Sprintf("关键词 %s 没有索引数据，直接返回空结果", word))
			rs := new(search.SearchResult)
			rs.Total = com.Uint32ToString(totalCount)
			rs.Count = "0"
			return rs
		}
	...
	if len(cond.Loglevels) > 0 {
		...
		if allcnt < 1 {
			global.LbLogger.Info(fmt.Sprintf("日志级别范围 %s 在日志仓 %s 中没有索引数据，直接返回空结果", com.Join(cond.Loglevels, ","), e.storeName))
			rs := new(search.SearchResult)
			rs.Total = com.Uint32ToString(totalCount)
			rs.Count = "0"
			return rs
	...
	if cond.OrgSystem == "" && cond.OrgSystems[0] != "*" {
		...
		if allcnt < 1 {
			global.LbLogger.Info(fmt.Sprintf("权限范围系统 %s 在日志仓 %s 中没有索引数据，直接返回空结果", com.Join(cond.OrgSystems, ","), e.storeName))
			rs := new(search.SearchResult)
			rs.Total = com.Uint32ToString(totalCount)
			rs.Count = "0"
			return rs
	...
	if len(cond.Kws) == 0 {
		...
		if cond.NewNearId > 0 {
			// 相邻检索模式的判断及条件准备
			max := global.LbConfig.Search.NearSearchSize
			...
			if len(rs.Data)-1 < max {
				rs.Count = com.IntToString(len(rs.Data) - 1)
			}
	...
	if maxMatchCount < com.StringToUint32(rs.Count, 0) {
		rs.Count = com.Uint32ToString(maxMatchCount) // 取其小，尽量减少最大匹配件数的误差
	}
	return rs
}
